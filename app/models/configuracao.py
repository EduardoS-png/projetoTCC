...
def get_configuracao():
    ...
    try:
        ...
        cursor.execute("SELECT id, nome, email, senha, status ativo FROM usuario ORDER BY nome")
...
def alterar(id, novoNome, novoEmail, novaSenha, novoStatus):
    ...
    try:
        cursor = conexao.cursor()
        sql = "UPDATE categoria set nome = %s , email = %s ,senha = %s , status= %s WHERE id = %s"
